...
- враги появляются сверху со стартовым интервалом 2 секунды;
- по мере игры интервал уменьшается до 0.2 секунды;
...
В игре есть процедурный спавн баффов. Они появляются вместе с волнами врагов и тоже двигаются сверху вниз. Чтобы активировать бафф, игрок должен расстрелять объект баффа до нуля HP.

Активные баффы в текущей сцене:

- Double Bullets: усиливает выстрел, переключая снаряд в двойной режим.
- Rapid Fire: ускоряет стрельбу игрока.
...
После поражения игрок может получить один респавн за просмотр rewarded-рекламы.

### Покупки в риложении

В интерфейсе предусмотрена кнопка Remove Ads. Для неё подготовлен сценарий покупки отключения рекламы.
...
- полоса здоровья на основе HP-иконок;
